...
```
* 매수/매도 요청도 메시지 큐를 통해 순차 처리
* TradeService에서 거래 처리 및 WebSocket 퍼블리시 수행

---
## 결론

Stockify에서는 **메시지 큐(RabbitMQ) 기반 순차 처리**, **비관적 락 + 트랜잭션**을 활용하여 동시성 문제를 효과적으로 관리했습니다.

* **메시지 큐(RabbitMQ)**: 모든 작업이 **순차적으로 안전하게 처리**
* **비관적 락 + 트랜잭션**: 데이터 **일관성 유지**, 충돌 방지
* **확장성과 유연성 확보**: 스케줄러 + 매수/매도 작업 안전한 처리 가능

> **결과**: 사용자 거래와 스케줄러 작업이 동시에 발생해도 **안정적 거래 환경** 보장
